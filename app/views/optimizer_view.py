...
def _spinbox_no_buttons_ss() -> str:
    """SpinBox stylesheet that hides the up/down arrow buttons entirely."""
    return f"""
        ...
        QDoubleSpinBox::up-button,   QSpinBox::up-button   {{ width: 0; height: 0; border: none; }}
...
class OptimizerView(QWidget):
    ...
    def _build_toolbar(self) -> QWidget:
        ...
        self._combo_circuit.setFont(QFont("Helvetica", 12))
        ...
        self._btn_optimize.setFont(QFont("Helvetica", 13, QFont.Weight.Bold))
    ...
    def _build_ga_panel(self) -> QWidget:
        ...
        btn_adv.setFont(QFont("Helvetica", 11, QFont.Weight.DemiBold))
        btn_adv.setStyleSheet(
            f"color: {TEXT_SUB}; font-size: 11px; font-weight: 600; "
            f"font-family: Helvetica; text-align: left; padding: 0; "
    ...
    def _build_fitness_panel(self) -> QWidget:
        ...
        self._spin_dir_penalty = self._labeled_spin_float(
            layout, "Direction penalty", 1.0, 10.0, 1.0,
        )
        ...
        self._spin_tolerance = self._labeled_spin_float(
            layout, "Tolerance band (%)", 0.0, 50.0, 0.0,
        )
    ...
    def _labeled_spin_float(
        ...
        spin.setDecimals(2)
        spin.setFont(QFont("Helvetica", 12))
        spin.setFixedHeight(34)
        spin.setStyleSheet(_spinbox_no_buttons_ss())
        layout.addWidget(lbl)
    ...
    def _build_target_inputs(self):
        ...
        for m in circuit["metrics"]:
            ...
            spin.setFont(QFont("Helvetica", 12))
            spin.setFixedHeight(34)
            # Target spec inputs: no arrow buttons
            spin.setStyleSheet(_spinbox_no_buttons_ss())
    ...
    def _on_generation(self, gen: int, best_score: float):
        ...
        now = time.monotonic()
        if now - self._last_chart_draw >= 0.067:
